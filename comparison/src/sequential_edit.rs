...
pub fn sequential_editing_rga(dataset_name: &str) {
    let data = load_testing_data(format!("comparison/editing_trace/{}.json.gz", dataset_name).as_str());
    let mut out = get_output_file(format!("sequential_editing_rga-{dataset_name}").as_str());
    ...
    writeln!(out, "The total number of operations is {:?}", total_op_count).unwrap();
...
pub fn sequential_editing_bft_rga(dataset_name: &str) {
    let data = load_testing_data(format!("comparison/editing_trace/{}.json.gz", dataset_name).as_str());
    let mut out = get_output_file(format!("sequential_editing_bft_rga-{dataset_name}").as_str());
